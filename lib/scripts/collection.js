...
window.checkValidJSON = function () {
  $.ajax({
    ...
    if (data === 'Valid') {
      ...
      $('#document-modal-body').parent().append('<div id="documentInvalidJSON" class="alert alert-danger"><strong>非法的JSON</strong></div>');
    }
  });
  return false;
};
...
window.checkValidIndexJSON = function () {
  $.ajax({
    ...
    if (data === 'Valid') {
      ...
      $('#index-modal-body').parent().append('<div id="indexInvalidJSON" class="alert alert-danger"><strong>非法的JSON</strong></div>');
    }
  });
  return false;
};
  ...
  $('.tooDamnBig').bind('click', function (e) {
    ...
    $.get(`${makeCollectionUrl()}${encodeURIComponent(_id)}/${prop}`, function (input) {
      ...
      if (
        ...
        input = '<audio controls style="width:45px;" src="' + input + '">您的浏览器不支持音频</audio>';
      }
